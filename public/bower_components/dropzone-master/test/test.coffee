  ...
  describe "init()", ->
    describe "clickable", ->
      dropzones =
        ...
        "using acceptedMimeTypes":
      new Dropzone(Dropzone.createElement("""<form action="/"></form>""" < form action = "/" > < /form>"""), { clickable: yes, acceptedMimeTypes: "audio/ *, video / * " })
      ...
      when
      something is selected to reset the input field",
      ->
        for i in [0..3]
    ...
    it "should create a .dz-message element", ->
      element = Dropzone.createElement """<form class="dropzone" action="/"></form>""" < form class = "dropzone"
      action = "/" > < /form>"""
